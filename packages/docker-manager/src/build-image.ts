...
import { getBaseImageTag } from './base-image'
...
export async function buildMockImage(params: BuildMockImageParams): Promise<{ imageId: string }> {
    ...
    try {
        ...
        // Base image has node + contour + patch already baked in.
        // This stage only adds the spec — should take 2-5 seconds.
        ...
        docker.modem.followProgress(stream, async (err: Error | null, output: Array<{ stream?: string; aux?: { ID: string } }>) => {
            ...
            for (let attempt = 1; attempt <= 5; attempt++) {
                try {
                    ...
                    if (attempt === 5) {
                        reject(new Error(`Build completed but image "${imageTag}" not found`))
                        return
                    }
